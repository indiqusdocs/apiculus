...
1. NAT Gateway Configuration Overview: The following details summarize the key configuration parameters of the selected NAT Gateway.
    ...
    - **PUBLIC IP**: For example `164.164.197.178` (The public-facing IP through which outbound traffic is routed.)
...
4. Access Internal Information for System Reference: The following internal configuration details are provided for system-level reference and tracking of the NAT Gateway within the  Cloud environment.
    ...
    - **VIRTUAL GATEWAY INTERNAL NAME**: `r-2524-VM`(Used internally by  for VM/gateway tracking and routing.)
...
4. Once inside the selected NAT Gateway, see a section titled **Port Forwarding** along with the public IP address (for example `164.164.197.178`).
...
2. Enter the **Public IP** of the pfsense WAN interface (for example `164.164.197.178`) and **port 22** in **PuTTY**.
