...
3. The newly created firewall (e.g., `ADC-R522-40-Test`) appears with:
    - Name
...
A green status icon **POWERED ON** confirms that the firewall is active and running.
...
2. On the firewall **Overview** page, you can view key details such as:
    ...
    - **Public IPv4 (L2-WAN)**: Note down the public IP (e.g., `103.145.166.60`) which is used for SSH access.
...
1. Open the Firewall Instance: From the Apiculus Cloud dashboard, navigate to the **Virtual Firewalls** section and open the required firewall instance (e.g., `ADC-R522-40-Test`).
...
3. Identify the **WAN Tier**: Look for the network labeled as `ISOLATED_NETWORK`. This is the WAN Tier (e.g., `ADC-R522-40-Test`), typically connected to external/public networks.
...
1. Copy the Public IP Address: From the firewall details page in Apiculus Cloud, note down the Public IPv4 (L2-WAN) address (e.g., `103.145.166.60`).
2. Open a Web Browser: Launch any web browser like Chrome, Firefox, or **Edge**.
3. Paste the IP address in the address bar (e.g., `https://103.145.166.60`) and press Enter.
...
4. Select Compute Configuration: Choose a VM configuration for example`DEMO-CC DCC-1C2R` 
5. Click Selected to confirm the compute pack.
...
1. **Choose an Authentication Method**:
    ...
    - **Use root user password** (Selected)
     
:::note
 Root passwords are less secure than SSH-based access
:::
...
2. **Name Your Linux Instance**:
    - `**Test-VNF-Ubuntu**` (Instance name can only contain alphanumeric characters, underscores, dots, and hyphens).
...
2. The window displays the **Hourly charge**
3. Click the **CONFIRM** button to proceed with the instance purchase.
4. If you do not want to proceed, click **CANCEL**.
